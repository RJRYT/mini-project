...
router.use(function (req, res, next) {
    ...
    else if (!req.session.user.Admin) res.status(403).sendFile(path.join(__dirname, '../Static/403.html'));
    else next();
});
...
router.use(async function (err, req, res, next) {
    ...
    if (!req.session.user.Admin)  res.status(500).sendFile(path.join(__dirname, '../Static/500.html'));
    else res.status(500).render('admin/500', { name: req.session.username, user: req.session.user, image: req.session.user.ProfilePic || "defaultpic.png" });
});
